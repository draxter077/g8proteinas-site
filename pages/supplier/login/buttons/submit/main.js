            ...
            if(e.target.innerHTML == "Entrar"){
                if(cnpj.value.length == 0 || password.value.length == 0 || cnpj.value == undefined || password.value == undefined){
                    ...
                    if(password.value == "esqueciminhasenha"){
                        await axios.post(apiURL + "/supplier/post/userForgotPassword", {cnpj:cnpj.value})
                        ...
                        await axios.post(apiURL + "/supplier/post/userLog", {cnpj:cnpj.value, password:password.value})
                            .then(r => {construct({page:"supplier", data:r.data})})
                            .catch(r => {
                                if(r.response.status == 404){
                                    showWindow("CNPJ não encontrado")
                                    inputError(cnpj)
                ...
                if(businessName.value.length == 0 || email.value.length == 0 || phoneNumber.value.length == 0 || cnpj.value.length == 0 || password.value.length == 0 || businessName.value == undefined || email.value == undefined || phoneNumber.value == undefined || cnpj.value == undefined || password.value == undefined){
                    ...
                    await axios.post(apiURL + "/supplier/post/userCreation", {cnpj:cnpj.value, businessname:businessName.value, email:email.value, phonenumber:phoneNumber.value, password:password.value})
                        .then(r => {construct({page:"supplier", data:r.data})})
                        .catch(r => {
